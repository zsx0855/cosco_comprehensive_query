...
logger = logging.getLogger("Vessel_Second_Hand_Disassemble_Risk_Screen")
api_logger = logging.getLogger("API_Requests")

# 路由器
disassemble_router = APIRouter(prefix="/second_hand", tags=["船舶二手拆解合规筛查"])
...
# 表名定义
SCREENING_LOG_TABLE = "lng.vessel_second_hand_disassemble_risk_log"

# 检查项类型映射关系
CHECK_ITEM_MAPPING = {
    "shipbreaking_party": "second_hand_shipbreaking_party",
    "shipbreaking_party_shareholder": "second_hand_shipbreaking_shareholder",
    "shipbreaking_party_controller": "second_hand_shipbreaking_controller",
...
class DisassembleRequest(BaseModel):
    ...
    # 相关方信息
    shipbreaking_party: str = Field(..., alias="shipbreaking_party")
    shipbreaking_party_shareholder: List[str] = Field(..., alias="shipbreaking_party_shareholder")
    shipbreaking_party_controller: str = Field(..., alias="shipbreaking_party_controller")
...
class DisassembleResponse(BaseModel):
    ...
    Vessel_name: str
    Vessel_imo: str
    Shipbreaking_party: StakeholderRisk
    Shipbreaking_party_shareholder: List[StakeholderRisk]
    Shipbreaking_party_controller: StakeholderRisk
    Agent: List[StakeholderRisk]
    Shipbreaking_party_fleet_supplied_Lloyd: List[FleetRiskItem]
    Shipbreaking_party_fleet_supplied_kpler: List[FleetRiskItem]
    Shipbreaking_party_fleet_supplied_uani: List[FleetRiskItem]
...
class OptimizedRiskCheckOrchestrator(RiskCheckOrchestrator):
    ...
    def execute_all_checks_optimized(self, vessel_imo: str, start_date: str, end_date: str,
                                    shipbreaking_party: str, shipbreaking_party_shareholder: List[str],
                                    shipbreaking_party_controller: str, agent: List[str],
                                    intent_transferee_fleet_supplied: List[Dict[str, str]]) -> List[CheckResult]:
        """执行所有检查项"""
        ...
        composite_results = self._execute_composite_checks(
            ...
            shipbreaking_party, shipbreaking_party_shareholder,
            shipbreaking_party_controller, agent,
    ...
    def _execute_composite_checks(self, vessel_imo: str, start_date: str, end_date: str, all_data: Dict[str, Any],
        ...
        composite_checks = [
            ("拆船方制裁风险检查", self.execute_dowjones_sanctions_risk_check, [shipbreaking_party], "shipbreaking_party_sanctions_risk"),
            ("拆船方股东制裁风险检查", self.execute_dowjones_sanctions_risk_check, [shipbreaking_party_shareholder], "shipbreaking_party_shareholder_sanctions_risk"),
            ("拆船方实控人制裁风险检查", self.execute_dowjones_sanctions_risk_check, [shipbreaking_party_controller], "shipbreaking_party_controller_sanctions_risk"),
...
def normalize_stakeholder_risk(risk_item: Union[Dict, Any], default_name: str = "") -> Dict:
    ...
    risk_type_mapping = {
        "shipbreaking_party": 1,
        "shipbreaking_party_shareholder": 2,
        "shipbreaking_party_controller": 3,
        "agent": 4
    }
...
# ---------------------- 3. 数据写入函数 ----------------------
def insert_disassemble_risk_log(request: DisassembleRequest, response: DisassembleResponse,
    ...
    try:
        ...
        insert_params = {
            ...
            # 相关方风险结果
            "shipbreaking_party_risk": Json(model_to_json(response.Shipbreaking_party)),
            "shipbreaking_party_shareholder_risk": Json(model_to_json(response.Shipbreaking_party_shareholder)),
            "shipbreaking_party_controller_risk": Json(model_to_json(response.Shipbreaking_party_controller)),
            "agent_risk": Json(model_to_json(response.Agent)),
            
            # 船队风险筛查结果
            "fleet_lloyd_risk": Json(model_to_json(response.Shipbreaking_party_fleet_supplied_Lloyd)),
            "fleet_kpler_risk": Json(model_to_json(response.Shipbreaking_party_fleet_supplied_kpler)),
            "fleet_uani_risk": Json(model_to_json(response.Shipbreaking_party_fleet_supplied_uani)),
        ...
        insert_sql = """
                     INSERT INTO lng.vessel_second_hand_disassemble_risk_log (
                         ...
                         shipbreaking_party_risk, shipbreaking_party_shareholder_risk, shipbreaking_party_controller_risk,
                         agent_risk,
                         fleet_lloyd_risk, fleet_kpler_risk, fleet_uani_risk,
                         ...
                         %(shipbreaking_party_risk)s, %(shipbreaking_party_shareholder_risk)s, %(shipbreaking_party_controller_risk)s,
                         %(agent_risk)s,
                         %(fleet_lloyd_risk)s, %(fleet_kpler_risk)s, %(fleet_uani_risk)s,
        ...
        with conn.cursor() as cursor:
            ...
            conn.commit()
        logger.info(f"船舶拆解风险数据写入成功（UUID: {request.Uuid}, IMO: {request.Vessel_imo}）")
...
# ---------------------- 4. 核心接口函数 ----------------------
@disassemble_router.post("/vessel_disassemble_risk", response_model=DisassembleResponse)
async def vessel_disassemble_risk_screen(request: DisassembleRequest):
    """船舶拆解合规状态筛查主接口"""
    try:
        current_time = get_current_local_time()
        ...
        vessel_imo = request.Vessel_imo.strip()
        vessel_name = request.Vessel_name.strip()
        logger.info(f"开始船舶拆解合规筛查（UUID: {request.Uuid}, IMO: {vessel_imo}, 名称: {vessel_name}）")
        ...
        check_results = risk_check_orchestrator.execute_all_checks_optimized(
            ...
            request.shipbreaking_party, request.shipbreaking_party_shareholder,
            request.shipbreaking_party_controller, request.agent,
            request.intent_transferee_fleet_supplied
        )
        ...
        # 处理相关方风险结果
        shipbreaking_party_risk = normalize_single_stakeholder_risk(
            check_results_dict.get("shipbreaking_party_sanctions_risk", {}),
            default_name=request.shipbreaking_party,
            risk_type="shipbreaking_party_sanctions_risk"
        )
        
        shipbreaking_party_shareholder_risk = normalize_stakeholder_risk_list(
            check_results_dict.get("shipbreaking_party_shareholder_sanctions_risk", []),
            default_names=request.shipbreaking_party_shareholder,
            risk_type="shipbreaking_party_shareholder_sanctions_risk"
        )
        
        shipbreaking_party_controller_risk = normalize_single_stakeholder_risk(
            check_results_dict.get("shipbreaking_party_controller_sanctions_risk", {}),
            default_name=request.shipbreaking_party_controller,
            risk_type="shipbreaking_party_controller_sanctions_risk"
        )
        ...
        # 处理船队风险筛查结果
        shipbreaking_party_fleet_supplied_Lloyd = []
        shipbreaking_party_fleet_supplied_kpler = []
        shipbreaking_party_fleet_supplied_uani = []
        ...
        for idx, fleet_vessel in enumerate(request.intent_transferee_fleet_supplied):
            ...
            shipbreaking_party_fleet_supplied_Lloyd.append(
                normalize_fleet_risk(fleet_vessel, lloyd_risk, risk_type="lloyd")
            )
            shipbreaking_party_fleet_supplied_kpler.append(
                normalize_fleet_risk(fleet_vessel, kpler_risk, risk_type="kpler")
            )
            shipbreaking_party_fleet_supplied_uani.append(
        ...
        # 相关方风险处理
        list_stakeholder_risks = shipbreaking_party_shareholder_risk + agent_risk
        single_stakeholder_risks = [shipbreaking_party_risk, shipbreaking_party_controller_risk]
        ...
        # 船队风险处理
        fleet_risks = shipbreaking_party_fleet_supplied_Lloyd + shipbreaking_party_fleet_supplied_kpler + shipbreaking_party_fleet_supplied_uani
        ...
        response = DisassembleResponse(
            ...
            Vessel_risk_status=vessel_risk_status,
            Stakeholder_risk_status=stakeholder_risk_status,
            Shipbreaking_party=shipbreaking_party_risk,
            Shipbreaking_party_shareholder=shipbreaking_party_shareholder_risk,
            Shipbreaking_party_controller=shipbreaking_party_controller_risk,
            Agent=agent_risk,
            Shipbreaking_party_fleet_supplied_Lloyd=shipbreaking_party_fleet_supplied_Lloyd,
            Shipbreaking_party_fleet_supplied_kpler=shipbreaking_party_fleet_supplied_kpler,
            Shipbreaking_party_fleet_supplied_uani=shipbreaking_party_fleet_supplied_uani,
        ...
        # 写入数据库
        insert_disassemble_risk_log(request, response, project_risk_status, vessel_risk_status, stakeholder_risk_status)

        logger.info(f"船舶拆解合规筛查完成（UUID: {request.Uuid}）")
        return response

    except Exception as e:
        logger.error(f"筛查接口异常（UUID: {request.Uuid}）: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"船舶拆解合规筛查失败: {str(e)}")


# ---------------------- 5. 服务启动配置 ----------------------
def create_app():
    """创建FastAPI应用"""
    app = FastAPI(title="船舶拆解合规状态筛查接口", version="1.0")
    ...
    app.include_router(disassemble_router)
    ...
    async def health_check():
        return {
            ...
            "database": "kingbase (lng.vessel_second_hand_disassemble_risk_log)"
        }

    @app.get("/", tags=["系统"])
    async def root():
        return {
            "message": "船舶拆解合规状态筛查接口",
            "version": "1.0",
            "api": {
                "method": "POST",
                "url": "/second_hand/vessel_disassemble_risk",
