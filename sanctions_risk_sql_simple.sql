...
associated_parties AS (
    SELECT 
        ...
        JSON_ARRAYAGG(
            JSON_BUILD_OBJECT(
                'SORP_ID', t.SORP_ID,
                'nmtoken_level', IF(t.SOURCE_TYPE = 'PERSON', t3.nmtoken_level, t2.nmtoken_level),
                'ENTITYNAME11', IF(t.SOURCE_TYPE = 'PERSON', t3.ENTITYNAME1, t2.ENTITYNAME1),
                'SOURCE_TYPE', t.SOURCE_TYPE,
                'RELATION_NM', t1.RELATION_NM
            )
        ) as other_list
    ...
    WHERE t.associate_id IN (13019177,625225,13696610,13696619,
                           13696675,13696701,13696707,13696728,13696747)
...
high_risk_objects AS (
    SELECT 
        ...
        JSON_BUILD_OBJECT(
            ...
            'risk_desc', sd.risk_desc,
            'risk_desc_info', sd.risk_desc_info,
            'info', sd.info,
            'tab', h.tab_data
        ) AS obj
...
-- 6.5 无风险对象（补充无风险项，tab 为空）
no_risk_objects AS (
    SELECT 
        n.entity_id,
        JSON_BUILD_OBJECT(
            'risk_type', n.risk_type,
...
SELECT 
    ...
    -- 高风险列表（每种risk_type仅一个对象，包含描述与tab）
    COALESCE(
        (
            SELECT JSON_ARRAYAGG(o.obj)
    ...
    -- 关联方信息
    COALESCE(ap.other_list, JSON_ARRAY()) AS other_list
