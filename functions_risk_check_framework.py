...
class CheckResult:
    ...
    risk_description: Optional[str] = None  # 风险描述
...
class DowJonesSanctionsRiskCheckItem(CompositeCheckItem):
    ...
    def _check_single_entity(self, entity_name: str) -> CheckResult:
        ...
        try:
            ...
            # 如果数据库中的风险等级为空或无效，根据制裁列表重新计算
            if not risk_status or risk_status not in ["高风险", "中风险", "无风险"]:
                ...
                # 重新计算风险等级：高风险 > 中风险 > 无风险
                if sanctions_list:
                    risk_status = "高风险"
                elif mid_sanctions_list:
                    risk_status = "中风险"
                else:
                    risk_status = "无风险"
            ...
            # 添加 risk_description 属性
            risk_description_parts = []
            if risk_status_reason.get('is_san'):
                risk_description_parts.append(f"{risk_status_reason['is_san']}")
            if risk_status_reason.get('is_sco'):
                risk_description_parts.append(f"{risk_status_reason['is_sco']}")
            if risk_status_reason.get('is_ool'):
                risk_description_parts.append(f"{risk_status_reason['is_ool']}")
            
            check_result.risk_description = f"在道琼斯的判定为：{', '.join(risk_description_parts)}" if risk_description_parts else ""
