...
def main():
    ...
    sql_statements = [
        # 第一段 SQL
        """
        truncate table lng.risk_dqs_test20250917;
insert into lng.risk_dqs_test20250917
    ...
    (SELECT id,
           ...
           FROM
               (select id,
                  ...
                  action, 
                  activestatus,
                  row_number() over(partition by entity_id order by entity_dt,create_time desc) rn
...
WHERE end_time = '' and SANCTIONS_NM !='') t3 ON t.id = t3.MAIN_ID
        LEFT JOIN -- 注册国
        ...
        # 第二段 SQL
        """
truncate table lng.dqs_entity_sanctions_test20250917;
insert into lng.dqs_entity_sanctions_test20250917
select entity_id,
       ...
       group_concat(DISTINCT case when NMTOKEN_LEVEL like '%SCO%' then 'SCO' ELSE NMTOKEN_LEVEL END ORDER BY case when NMTOKEN_LEVEL like '%SCO%' then 'SCO' ELSE NMTOKEN_LEVEL END ASC SEPARATOR ';' ) AS NMTOKEN_LEVEL,
       CASE
...
group by entity_id
        ...
        # 第三段 SQL
        """
        truncate table lng.dqs_person_sanctions_test20250917;
        insert into lng.dqs_person_sanctions_test20250917
select entity_id,
       group_concat(DISTINCT entity_dt ORDER BY entity_dt ASC SEPARATOR ';' )                                                                                                                           AS entity_dt,
       group_concat(DISTINCT activestatus ORDER BY activestatus ASC SEPARATOR ';' )                                                                                                                     AS activestatus,
       group_concat(DISTINCT ENTITYNAME1 ORDER BY ENTITYNAME1 ASC SEPARATOR ';' )                                                                                                                       AS ENTITYNAME1,
          ...
          (SELECT id,
           ...
           FROM
               (select id,
                  ...
                  action,
                  activestatus,
                  row_number() over(partition by person_id order by entity_dt,create_time desc) rn
